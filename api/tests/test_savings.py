...
class TestCreateSavingsEndpoint:
    ...
    async def test_create_savings_success(
        self,
        mock_response,
        mock_get_current_account_data,
        mock_savings_repository,
    ):
        ...
        savings_input = SavingsIn(
            current_amount_saved=0.00, final_goal_amount=5000.00, plans_id=1
        )

        expected_savings_output = SavingsOut(id=1, **savings_input.dict())
        mock_repo_instance.create.return_value = expected_savings_output

        created_savings = await create_savings(
            savings_input, mock_response_instance, mock_repo_instance
        )
    ...
    @patch("routers.savings.authenticator.get_current_account_data")
    @patch("routers.savings.Response")
    async def test_create_savings_failure(
        self,
        mock_response,
        mock_get_current_account_data,
        mock_savings_repository,
