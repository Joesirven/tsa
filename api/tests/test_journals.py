...
class TestCreateJournalEndpoint:
    ...
    async def test_create_journal_success(self, mock_response, mock_get_current_account_data, mock_journal_repository):
        ...
        mock_response_instance.create.return_value = expected_journal_output  # Mocking the return value

        created_journal = await create_journal(journal_input, mock_response_instance, mock_repo_instance)
    ...
    @patch('routers.journals.authenticator.get_current_account_data')
    @patch('routers.journals.Response')
    async def test_create_journal_failure(self, mock_response, mock_get_current_account_data, mock_journal_repository):
        mock_repo_instance = mock_journal_repository.return_value
        mock_get_current_account_data.return_value = {"user_id": 1}
        mock_response_instance = mock_response.return_value
        ...
        mock_response_instance.create.return_value = expected_error  # Mocking the return value
